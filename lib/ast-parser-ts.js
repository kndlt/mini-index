...
function extractExports(filePath) {
  try {
    ...
    traverseForExports(ast, exports, filePath);
 ...
 */
function traverseForExports(ast, exports, filePath) {
  estraverse.traverse(ast, {
    enter: function(node) {
      ...
      if (node.type === 'TSInterfaceDeclaration' && node.id && node.id.name) {
        // Check if this interface is exported (should be handled by ExportNamedDeclaration above)
        // This is a fallback for standalone interfaces
      }

      if (node.type === 'TSTypeAliasDeclaration' && node.id && node.id.name) {
        // Check if this type is exported (should be handled by ExportNamedDeclaration above)
        // This is a fallback for standalone types
      }
